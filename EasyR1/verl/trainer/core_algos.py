...
@torch.no_grad()
def compute_tgrpo_outcome_advantage(
    token_level_rewards: torch.Tensor, eos_mask: torch.Tensor, index: torch.Tensor,
    token_level_rewards_shuffle: torch.Tensor, eos_mask_shuffle: torch.Tensor,
    epsilon: float = 1e-6
) -> Tuple[torch.Tensor, torch.Tensor]:
    """
    Compute advantage for GRPO, operating only on Outcome reward
    ...
    # shuffle
    scores_shuffle = token_level_rewards_shuffle.sum(dim=-1)
    id2score_shuffle = defaultdict(list)
    id2mean_shuffle, id2std_shuffle = {}, {}
    for i in range(bsz):
        id2score_shuffle[index[i]].append(scores_shuffle[i])
    
    for idx in id2score_shuffle:
        if len(id2score_shuffle[idx]) == 1:
            id2mean_shuffle[idx] = torch.tensor(0.0)
            id2std_shuffle[idx] = torch.tensor(1.0)
        elif len(id2score_shuffle[idx]) > 1:
            id2mean_shuffle[idx] = torch.mean(torch.tensor(id2score_shuffle[idx]))
            id2std_shuffle[idx] = torch.std(torch.tensor([id2score_shuffle[idx]]))
        else:
            raise ValueError(f"no score in prompt index: {idx}")

    # ----------------- add code here: -----------------
    for k,v in id2mean.items():
        assert k in id2mean_shuffle
        if  0.8 * id2mean[k] > id2mean_shuffle[k] and id2mean[k] > 0.3:
            for i in range(len(id2score[k])):
                if id2score[k][i] > 0.1:
                    id2score[k][i] += 0.3
        elif id2mean[k] < 0.3: # too hard, not change it
            pass
        else: # 0.8 * id2mean[k] < id2mean_shuffle[k] and id2mean[k] > 0.3, it means id2mean_shuffle[k] > 0.375. zero-out
            for i in range(len(id2score[k])):
                id2score[k][i] = 0
        
                
    # recompute id2mean and id2std
    id2mean = {}
    id2std = {}
    for k,v in id2score.items():
        id2mean[k] = torch.mean(torch.tensor(v))
        id2std[k] = torch.std(torch.tensor(v))
