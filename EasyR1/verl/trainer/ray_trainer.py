...
from ..utils.dataset_video_v1 import RLHFDataset, collate_fn
...
def compute_advantage(data: DataProto, adv_estimator: AdvantageEstimator, gamma=1.0, lam=1.0):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
    elif adv_estimator == AdvantageEstimator.TGRPO:
        # normal forward
        token_level_rewards = data.batch["token_level_rewards"]
        index = data.non_tensor_batch["uid"]
        responses = data.batch["responses"]
        ...
        response_mask = attention_mask[:, -response_length:]
        # shuffle forward
        token_level_rewards_shuffle = data.batch["token_level_rewards_shuffle"]
        responses_shuffle = data.batch["responses_shuffle"]
        response_length_shuffle = responses_shuffle.size(-1)
        attention_mask_shuffle = data.batch["attention_mask_shuffle"]
        response_mask_shuffle = attention_mask_shuffle[:, -response_length_shuffle:]

        advantages, returns = core_algos.compute_tgrpo_outcome_advantage(
            token_level_rewards=token_level_rewards, eos_mask=response_mask, index=index,
            token_level_rewards_shuffle=token_level_rewards_shuffle,
            eos_mask_shuffle=response_mask_shuffle,
        )
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
...
class RayPPOTrainer:
    ...
    def _load_checkpoint(self) -> None:
        ...
        base = Path(load_path)
        print(f"Trying to load checkpoint from {base}")

        # 1) Base path must exist and be a directory
        if not base.is_dir():
            print(f"Checkpoint path {base} not found; starting from scratch.")
            return

        # 2) Find all sub‑dirs named "global_step_<n>"
        candidates = []
        for sub in base.glob("global_step_*"):
            if not sub.is_dir():
                continue
            # peel off the prefix and ensure it’s numeric
            suffix = sub.name.removeprefix("global_step_")
            if not suffix.isdigit():
                continue
            candidates.append((int(suffix), sub))

        if not candidates:
            print(f"No 'global_step_<n>' dirs in {base}; starting from scratch.")
            return

        # 3) Pick the highest-numbered checkpoint
        latest_step, latest_dir = max(candidates, key=lambda x: x[0])
        print(f"Loading checkpoint at step {latest_step} from {latest_dir}")

        # 4) Update config to point at that folder
        self.config.trainer.load_checkpoint_path = str(latest_dir)
        ...
        for _ in range(self.config.trainer.total_episodes):
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    if self.config.algorithm.adv_estimator == "tgrpo":
                        with _timer("gen_shuffle", timing_raw):
                            gen_batch_shuffle = deepcopy(gen_batch)
                            for i in range(len(gen_batch_shuffle)):
                                num_videos = len(gen_batch_shuffle[i].non_tensor_batch['multi_modal_data']['video'])
                                for j in range(num_videos):
                                    gen_batch_shuffle[i].non_tensor_batch['multi_modal_data']['video'][j] = gen_batch_shuffle[i].non_tensor_batch['multi_modal_data']['video'][j][torch.randperm(gen_batch_shuffle[i].non_tensor_batch['multi_modal_data']['video'][j].shape[0])]
                            gen_batch_shuffle_output = self.actor_rollout_wg.generate_sequences(gen_batch_shuffle)
                            del gen_batch_shuffle
                    ...
                    if self.config.algorithm.adv_estimator == "tgrpo":
                        # rename the key of the gen_batch_shuffle_output, add '_shuffle' to each key
                        # ValueError: Expected a TensorDictBase instance but got <class 'torch.Tensor'> instead for key '('input_ids', 'responses', 'position_ids', 'prompts')' in tensordict:
                        gen_batch_shuffle_output_batch = gen_batch_shuffle_output.pop(
                            batch_keys=['attention_mask', 'input_ids', 'responses', 'position_ids', 'prompts'],
                        ) 
                        # convert the input to list of tuples
                        for key in ['attention_mask', 'input_ids', 'responses', 'position_ids', 'prompts']:
                            gen_batch_shuffle_output_batch.rename(old_keys=key, new_keys=f"{key}_shuffle")
                        batch = batch.union(gen_batch_shuffle_output_batch)
                        del gen_batch_shuffle_output
                        # del gen_batch_shuffle_output_batch
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        if self.config.algorithm.adv_estimator == "tgrpo":
                            reward_tensor, reward_tensor_shuffle = self.reward_fn(batch)
                            batch.batch["token_level_scores"] = reward_tensor
                            batch.batch["token_level_scores_shuffle"] = reward_tensor_shuffle
                        else:
                        ...
                        if not self.config.worker.actor.use_kl_loss:  # not grpo
                            ...
                            if self.config.algorithm.adv_estimator == "tgrpo":
                                batch.batch["token_level_rewards_shuffle"] = batch.batch["token_level_scores_shuffle"]
